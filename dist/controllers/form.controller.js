    ...
    constructor() {
        ...
        this.getFormSubmissions = async (req, res) => {
            try {
                ...
                const { page = 1, limit = 50, utmSource, utmCampaign, leadStatus, startDate, endDate, formName } = req.query;
                // Find connection
                ...
                if (formName)
                    where.wixFormName = { contains: formName, mode: 'insensitive' };
                ...
                // Get total count
                const total = await database_1.prisma.formSubmission.count({ where });
                // Get paginated submissions
                const submissions = await database_1.prisma.formSubmission.findMany({
                    where,
                ...
                const stats = await database_1.prisma.formSubmission.groupBy({
                    by: ['leadStatus'],
                    where: { connectionId: connection.id },
                    _count: true,
                });
                const utmStats = await database_1.prisma.formSubmission.groupBy({
         ...
         */
        this.getFormSubmissionById = async (req, res) => {
            try {
                const { submissionId } = req.params;
                const submission = await database_1.prisma.formSubmission.findUnique({
         ...
         */
        this.getFormStats = async (req, res) => {
            try {
                const { instanceId } = req.params;
                ...
                const submissionsByDayRaw = await database_1.prisma.$queryRaw `
        SELECT 
      ...
      `;
                ...
                // Get top sources
                const topSources = await database_1.prisma.formSubmission.groupBy({
                    by: ['utmSource'],
                    where: {
                ...
                // Get top campaigns
                const topCampaigns = await database_1.prisma.formSubmission.groupBy({
                    by: ['utmCampaign'],
                    where: {
                        connectionId: connection.id,
                ...
                const totalSubmissionsRaw = await database_1.prisma.formSubmission.count({
                    where: { connectionId: connection.id }
                });
                const last7DaysRaw = await database_1.prisma.formSubmission.count({
                    where: {
                        connectionId: connection.id,
        ...
        this.updateLeadStatus = async (req, res) => {
            try {
                ...
                const submission = await database_1.prisma.formSubmission.update({
                    where: { id: submissionId },
                    data: updateData,
                });
                res.json({
